...
function App() {
	...
	function addPersonDetails() {
		setPersonList([
			...personList,
			{ id: personList.length + 1, name: personName.current.value, email: personEmail.current.value }
		]);
	...
	function getPersonList() {
		return personList
			.filter((person) => (currentSearch === '' ? true : person.name.includes(currentSearch)))
			.map((person) => {
				return (
				...
				<div>
					...
					{personDetailsView == 'EditView' && (
						<div>
							...
							<div>
								name: <input type="text" defaultValue={personEdit.name} ref={personName} />
							</div>
							<div>
								email: <input type="text" defaultValue={personEdit.email} ref={personEmail} />
							</div>
							<button
								style={{ marginTop: '10px' }}
								type="button"
